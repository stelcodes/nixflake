    ...
    programs = {
      ...
      yazi = {
        ...
        settings = {
          ...
          opener = {
            ...
            dj = lib.mkIf (config.profile.graphical && config.activities.djing) [
              ...
              {
                desc = "Rekordbox";
                run = "${pkgs.rekordbox-add}/bin/rekordbox-add \"$@\"";
                block = true;
                for = "unix";
              }
              {
                desc = "Convert audio";
                run = "${pkgs.convert-audio}/bin/convert-audio \"$1\"";
                block = true;
                for = "unix";
              }
            ];
        ...
        plugins =
          let
            yp = pkgs.yaziPlugins;
            _mkYaziPlugin = (initLua: "${(pkgs.writeTextDir "plugin/main.lua" initLua)}/plugin");
          in
          {
            chmod = yp.chmod;
